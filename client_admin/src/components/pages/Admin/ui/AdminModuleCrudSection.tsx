        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {rows.length === 0 ? <tr><td colSpan={tableCols.length + 1} className="px-4 py-8 text-center text-slate-500">Không có dữ liệu</td></tr> : rows.map((row, idx) => (
              <tr key={idx} className="border-t border-slate-200">{tableCols.map((col) => <td key={col} className="px-4 py-3">{String(row[col] ?? "")}</td>)}<td className="px-4 py-3"><div className="flex gap-2"><button onClick={() => { const id = String(row.id ?? ""); if (!id) return; if (basePath) { navigate(`${basePath}/${moduleKey}/${id}/edit`); return; } applyRow(row); setActivePanel("update"); setResultText("Sẵn sàng thao tác"); }} className="rounded border border-slate-300 px-2 py-1">Sửa</button><button onClick={() => { const id = String(row.id ?? ""); if (!id) return; if (basePath) { navigate(`${basePath}/${moduleKey}/${id}`); return; } setDetailId(id); setActivePanel("detail"); setResultText("Sẵn sàng thao tác"); }} className="rounded border border-slate-300 px-2 py-1">Xem</button><button onClick={() => { setDeleteId(String(row.id ?? "")); setActivePanel("delete"); setResultText("Sẵn sàng thao tác"); }} className="rounded border border-rose-300 px-2 py-1 text-rose-700">Xóa</button></div></td></tr>
            ))}
          </tbody>
        </table>
      </div>
